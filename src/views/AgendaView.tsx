...
export const AgendaView: React.FC = () => {
  ...
  return (
    <div id="agenda-section-view" className="space-y-6 pb-12">
      ...
      <div className="flex items-center gap-2 overflow-x-auto pb-1" id="agenda-filters-container">
        ...
        {filterOptions.map((opt) => (
          <button
            ...
            className={`px-3 py-1.5 rounded-full text-xs font-semibold whitespace-nowrap transition-colors cursor-pointer ${selectedFilter === opt.id
      ...
      {/* 3.2 VISTA SEMANA */}
      {viewMode === 'semana' && (
        <div id="agenda-week-view-container" className="bg-white dark:bg-slate-900 rounded-lg border border-slate-200 dark:border-slate-800 p-4 overflow-x-auto">
          <div className="min-w-[760px]">
            <div className="grid grid-cols-7 gap-2 pb-3 border-b border-slate-200 dark:border-slate-800 text-center font-bold text-xs">
              {weekDays.map((d) => (
                <div
                  key={d.dayNum}
                  className={`p-2 rounded-lg ${d.dayNum === 3
            ...
            <div className="grid grid-cols-7 gap-2 pt-3 min-h-[380px]">
              {weekDays.map((d) => {
                ...
                return (
                  <div
                    key={d.dayNum}
                    className="flex flex-col gap-2 p-2 rounded-lg bg-slate-50/50 dark:bg-slate-800/30 border border-slate-100 dark:border-slate-800/50"
                  >
                    {colEvents.length === 0 ? (
      ...
      {viewMode === 'mes' && (
        <div id="agenda-month-view-container" className="space-y-4">
              ...
              {Array.from({ length: 30 }, (_, i) => i + 1).map((dayNum) => {
                ...
                return (
                  <button
                    ...
                    className={`min-h-[64px] p-2 rounded-lg text-left flex flex-col justify-between border transition-all cursor-pointer ${isSelected
          ...
          <div className="p-5 bg-slate-50 dark:bg-slate-800/50 rounded-lg border border-slate-200 dark:border-slate-800">
                ...
                {filteredEvents
                  ...
                  .map((ev) => (
                    <div
                      key={ev.id}
                      onClick={() => openModal('event_detail', ev)}
                      className="p-3 bg-white dark:bg-slate-900 rounded-lg border border-slate-200 dark:border-slate-800 flex items-center justify-between cursor-pointer hover:border-brand/40 transition-colors"
                    >
                      <div className="flex items-center gap-3">
